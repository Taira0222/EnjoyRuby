class FullTimeEmployee 
    ...
    def initialize(name,age,skill_level)
        ...
        @base_sarary = SKILL_LEVEL_SARARY[skill_level]
    end

    # 給与計算
    def calulate_sarary
        @base_sarary + @age * AGE_BOUNUS
    end
    # 給与明細を出力
    def generate_pay_slip
        puts "Name: #{@name}\nSalary: #{calulate_sarary}円"
    end
end
...
class PartTimeEmployee
    ...
    def generate_pay_slip
        puts "Name: #{@name}\nSalary: #{calulate_sarary.round}円" #　浮動小数点は四捨五入をする
    end
end
...
class ContractEmployee
    ...
    def calulate_sarary
        if @hours_worked > 160
            return HOURLY_RATE * (160 + (@hours_worked - 160)* OVER_RATE ) # 働いた時間から160時間をオーバーした時間に残業代をかける
        else
